...
install-common:
	@[ "$(WITH_BASHCOMP)" = "yes" ] || exit 0; install -v -d "$(DESTDIR)$(BASHCOMPDIR)" && install -m 0644 -v src/completion/passage.bash-completion "$(DESTDIR)$(BASHCOMPDIR)/passage"
...
install: install-common
	...
	@trap 'rm -f src/.passage' EXIT; sed 's:.*PLATFORM_FUNCTION_FILE.*:source "$(LIBDIR)/passage/platform.sh":;s:^SYSTEM_EXTENSION_DIR=.*:SYSTEM_EXTENSION_DIR="$(LIBDIR)/passage/extensions":' src/passage.sh > src/.passage && \
